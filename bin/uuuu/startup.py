    ...
    try:
        if BOTLOG:
            Config.ZEDUBLOGO = await zedub.tgbot.send_file(
                BOTLOG_CHATID,
                "https://vm.tiktok.com/ZM6MHPCun/",
                caption="**•⎆┊تـم بـدء تشغـيل سـورس الفراعنة  الخاص بك .. بنجاح 🧸♥️**",
                buttons=[(Button.url("𖣳سورس الفراعنة 𖣳", "https://t.me/wasit_go"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
...
async def mybot():
    ...
    Malath = bot.uid
    co_od = f"[{ZELZAL}](tg://user?id={Malath})"
    f"ـ {co_od}"
    f"•⎆┊هــذا البــوت خــاص بـ {co_od} يمكـنك التواصــل معـه هـنا 🧸♥️"
    ...
    if bot_name.endswith("Assistant"):
        ...
        try:
            ...
            await asyncio.sleep(1)
            await bot.send_message("@BotFather", "roza")
            ...
            await bot.send_message("@BotFather", botname)
            await asyncio.sleep(1)
            await bot.send_message("@BotFather", f"- بـوت الفراعنة  المسـاعـد ♥️🦾 الخـاص بـ  {bot.me.first_name} ")
            await asyncio.sleep(3)
            await bot.send_message("@BotFather", "/setdescription")
            await asyncio.sleep(1)
            await bot.send_message("@BotFather", botname)
            await asyncio.sleep(1)
            await bot.send_message("@BotFather", f"•⎆┊انـا البــوت المسـاعـد الخــاص بـ {co_od} \n•⎆┊بـواسطـتـي يمكـنك التواصــل مـع مـالكـي 🧸♥️\n•⎆┊قنـاة السـورس 🌐 @wasit_go 🌐")
        except Exception as e:
            print(e)
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        _, groupid = await create_supergroup(
            "كـروب السجـل الفراعنة ", zedub, Config.TG_BOT_USERNAME, descript, photozed
        )
    ...
    if PM_LOGGER_GROUP_ID != -100:
        ...
        _, groupid = await create_supergroup(
            "كࢪوب التخـزين", zedub, Config.TG_BOT_USERNAME, descript, photozed
        )
